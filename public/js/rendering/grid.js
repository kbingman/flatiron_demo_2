function Grid(domID, width, height, scale){  
  ...
  self.init = function(){
    self.paper = Sector.paper;
    // self.paper = Raphael(domID, width, height);
    self.gridColor = 'rgba(255,255,255,0.18)';
    self.textColor = 'rgba(255,255,255,0.23)';
    self.scale = scale || 1;
    ...
    x_units.times(function(i){
      ...
      if(scale >= 0.5){
        y_units.times(function(y_i){
          ...
          var label = self.paper.text(x_label + 8,y_label + 12, (x_label) + ' : ' + (y_label)); // + Sector.starfield.x  + Sector.starfield.y
          label.attr({
            'fill': self.textColor,
            'font-size': 11, 
            'text-anchor': 'start',
